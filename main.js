...
function makeHostileMarkTexture(){const canvas=document.createElement('canvas');canvas.width=128;canvas.height=96;const ctx=canvas.getContext('2d');ctx.clearRect(0,0,128,96);ctx.shadowColor='#ff1b08';ctx.shadowBlur=14;for(let i=0;i<3;i++){const x=28+i*35,top=12+Math.abs(i-1)*8;ctx.beginPath();ctx.moveTo(x-10,top);ctx.lineTo(x+9,top+5);ctx.lineTo(x+3,top+34);ctx.lineTo(x-3,top+72);ctx.lineTo(x-9,top+38);ctx.closePath();ctx.fillStyle=i===1?'#ff4a20':'#c81208';ctx.fill();ctx.shadowBlur=4;ctx.beginPath();ctx.moveTo(x-2,top+8);ctx.lineTo(x+4,top+10);ctx.lineTo(x,top+43);ctx.lineTo(x-4,top+28);ctx.closePath();ctx.fillStyle='#ffd0a4';ctx.fill();ctx.shadowBlur=14;}const texture=new THREE.CanvasTexture(canvas);texture.colorSpace=THREE.SRGBColorSpace;texture.minFilter=THREE.LinearFilter;return texture;}
const hostileMarkTexture=makeHostileMarkTexture();
function addHostileMark(enemy,index=0){const marker=new THREE.Group(),height=enemy.userData.type==='tree'?14:enemy.userData.type==='car'?7:enemy.userData.segments?2.8:6.2,material=new THREE.SpriteMaterial({map:hostileMarkTexture,transparent:true,opacity:.78,depthWrite:false,depthTest:false,fog:false}),sprite=new THREE.Sprite(material);sprite.position.y=height;sprite.scale.set(2.85,2.15,1);sprite.renderOrder=999;marker.add(sprite);marker.userData={phase:index*.61,materials:[material],sprite,baseY:0};enemy.add(marker);hostileMarks.push(marker);return marker;}
...
function tick(){
  ...
  hostileMarks.forEach(marker=>{const pulse=Math.max(0,Math.sin(t*4.2+marker.userData.phase)),breath=1+pulse*.12;marker.position.y=Math.sin(t*3.1+marker.userData.phase)*.16;marker.userData.sprite.scale.set(2.85*breath,2.15*breath,1);marker.userData.materials.forEach(material=>material.opacity=.68+pulse*.3);});
