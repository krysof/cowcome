...
function resolvePlayerDynamicSolids(){
  activeChasers.forEach(enemy=>{if(!enemy.visible)return;const scale=enemy.userData.sizeMultiplier||1;pushPlayerFromPoint(enemy.position.x,enemy.position.z,enemy.userData.type==='car'?4.8:2.15*scale);});
  snakes.forEach(snake=>{if(snake.visible)pushPlayerFromPoint(snake.position.x,snake.position.z,2.05);});if(snowGhost.visible)pushPlayerFromPoint(snowGhost.position.x,snowGhost.position.z,2.3);
...
const scalableEnemies=[...wolfPack,...stalkers,...ambushers];scalableEnemies.forEach((enemy,i)=>{enemy.userData.baseScale=enemy.scale.clone();enemy.userData.giantRank=i%3===0?2+(Math.floor(i/3)%3):99;enemy.userData.sizeMultiplier=1;});
...
function tick(){
  ...
  if(state==='playing'){
    ...
    herdCows.forEach((cow,i)=>{
      ...
      const chase=new THREE.Vector3().subVectors(cow.position,enemy.position),d=chase.length();if(updateEnemyPounce(enemy,dt))return;if(d<7.2&&!cow.userData.super&&beginEnemyPounce(enemy,cow,true))return;enemy.position.addScaledVector(chase.normalize(),(cow.userData.super?7.1:7.05+i*.08)*dt);enemy.rotation.y=Math.atan2(-chase.x,-chase.z);animateCow(enemy,t,11);
    ...
    for(const enemy of activeChasers){
      ...
      if(targetNpc&&d<(enemy.userData.type==='car'?5.4:2.65*enemySize)){bloodyAttack(targetNpc,enemy);continue;}
