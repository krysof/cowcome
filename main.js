...
function addShoveFinger(character,skin,x=.9,y=2.55){
  const hands=[-1,1].map(side=>{
    ...
    const finger=new THREE.Group();finger.position.set(0,.08,-.08);finger.rotation.x=-Math.PI/2;finger.scale.y=.12;rig.add(finger);
    mesh(new THREE.CapsuleGeometry(.13,.5,3,8),skin,finger,0,.36,0,[1,1,1]);
    mesh(new THREE.CapsuleGeometry(.115,.46,3,8),skin,finger,0,.91,0,[.96,1,.96]);
    mesh(new THREE.CapsuleGeometry(.105,.38,3,8),skin,finger,0,1.39,0,[.92,1,.92]);
    const tip=mesh(new THREE.SphereGeometry(.13,9,7),skin,finger,0,1.68,0,[.92,1.08,.95]);
    const nailMat=new THREE.MeshStandardMaterial({color:0xffe7da,roughness:.58,metalness:0,flatShading:false});mesh(new THREE.SphereGeometry(.085,9,6),nailMat,finger,0,1.72,-.085,[.72,1.08,.22]);
    const creaseMat=new THREE.MeshBasicMaterial({color:0x8f654f,transparent:true,opacity:.55,fog:false});for(const jointY of [.68,1.18])mesh(new THREE.TorusGeometry(.112,.012,5,10,Math.PI*.9),creaseMat,finger,0,jointY,.012,[1,1,1],[Math.PI/2,0,.15]);
    return{rig,finger,tip,side};
  });
