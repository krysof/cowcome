...
function start(){
  ...
  const oldPlayer=player;player=createCharacter(selectedCharacter);player.position.set(0,.05,18);player.rotation.set(0,0,0);scene.remove(oldPlayer);scene.add(player);renderHearts();storyStage=0;waveWarningStage=0;activeChasers=[];allEnemies.forEach(e=>{e.visible=false;e.position.y=e.userData.type==='car'?0:.05;e.userData.feeding=0;e.userData.joined=false;e.userData.headBob=0;e.userData.spawned=false;});scalableEnemies.forEach(enemy=>{enemy.userData.sizeMultiplier=1;enemy.scale.copy(enemy.userData.baseScale);});stalkers.forEach(e=>e.position.copy(e.userData.home));monsterCar.position.set(0,0,80);snakes.forEach(snake=>{snake.position.copy(snake.userData.home);snake.rotation.set(0,Math.PI/2,0);snake.userData.avoidTimer=0;snake.visible=true;});treeEnemies.forEach(e=>e.visible=true);
...
function tick(){
  ...
  if(state==='playing'){
    ...
    let nearest=999;
    if(snowGhost.visible){const sanctuary=activeSafeZone>=0?safeZones[activeSafeZone]:(sheltered?shelterHuts.find(hut=>Math.hypot(player.position.x-hut.position.x,player.position.z-hut.position.z)<8):null);if(safeProtected&&sanctuary)patrolSanctuary(snowGhost,sanctuary,dt,t,activeSafeZone>=0?26:9);else{snowGhost.userData.patrolSanctuary=-1;const ghostV=new THREE.Vector3().subVectors(player.position,snowGhost.position);ghostV.y=0;const ghostD=ghostV.length();nearest=Math.min(nearest,ghostD);snowGhost.position.addScaledVector(ghostV.normalize(),(8.4+elapsed*.018)*difficulty.enemy*dt);snowGhost.rotation.y=Math.atan2(-ghostV.x,-ghostV.z);if(ghostD<2.75)damageHeart('event.heartLost',snowGhost,'death.snow');}snowGhost.position.y=.18+Math.sin(t*2.4)*.28;snowGhost.rotation.z=Math.sin(t*3.1)*.035;}
