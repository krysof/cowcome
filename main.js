...
function resolvePlayerDynamicSolids(){
  ...
  allEnemies.forEach(enemy=>{if(!enemy.visible)return;const scale=enemy.userData.sizeMultiplier||1,radius=enemy.userData.type==='car'?4.8:enemy.userData.segments?2.05:enemy.userData.collisionRadius?enemy.userData.collisionRadius+1.05:2.15*scale;pushPlayerFromPoint(enemy.position.x,enemy.position.z,radius);});if(snowGhost.visible)pushPlayerFromPoint(snowGhost.position.x,snowGhost.position.z,2.3);
