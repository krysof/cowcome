...
initI18n();
const isDesktopApp=/CowcomeMac\//.test(navigator.userAgent);
const isNativeApp=Capacitor.isNativePlatform()||isDesktopApp;
...
const characterKeys={orange:'character.orangeFull',yellow:'character.yellowFull',leopard:'character.leopardFull',super:'character.superFull'};
const difficultyKeys={orange:'character.easy',yellow:'character.normal',leopard:'character.hard',super:'character.hidden'};
const MEMORY_IDS=['bell','ribbon','photo','hoofprint','radio','snow','gate','mother'];
const PROGRESS_KEY='niulai-progress-v1';
function readProgress(){try{const value=JSON.parse(localStorage.getItem(PROGRESS_KEY)||'{}');return{memories:Array.isArray(value.memories)?value.memories.filter(id=>MEMORY_IDS.includes(id)):[],runs:Number(value.runs)||0};}catch{return{memories:[],runs:0}}}
let persistentProgress=readProgress();
function saveProgress(){try{localStorage.setItem(PROGRESS_KEY,JSON.stringify(persistentProgress));}catch{}renderProgress();}
function renderProgress(){const unlocked=persistentProgress.memories.length>=6;memoryCount.textContent=`${persistentProgress.memories.length} / ${MEMORY_IDS.length}`;superCharacter.classList.toggle('locked',!unlocked);superCharacter.setAttribute('aria-disabled',String(!unlocked));document.querySelector('#superUnlockText').textContent=tr(unlocked?'character.superUnlocked':'character.superLocked');memoryList.innerHTML=MEMORY_IDS.map((id,i)=>{const found=persistentProgress.memories.includes(id);return `<div class="memory-entry ${found?'':'locked'}"><b>${found?tr(`memory.${id}.title`):tr('memory.unknown',{number:i+1})}</b><small>${found?tr(`memory.${id}.description`):tr('memory.locked')}</small></div>`}).join('');}
function collectMemory(id){if(persistentProgress.memories.includes(id))return false;persistentProgress.memories.push(id);saveProgress();say('memory.found',2900,{title:tr(`memory.${id}.title`)});return true;}
...
function renderScores(){
  const scores=readScores();scoreList.innerHTML=scores.length?scores.map(s=>`<li><div><b>${tr(characterKeys[s.character]||'character.niulai')}</b><small>${tr(difficultyKeys[s.character]||'character.easy')} · ${tr(s.win?'scores.escaped':'scores.caught')} · ${tr('scores.meter',{distance:s.distance})}</small><small>${tr('scores.details',{ending:tr(s.endingKey||'ending.loop'),rescued:s.rescuedCows||0,collected:s.collected||0})}</small></div><strong>${formatTime(s.time)}</strong></li>`).join(''):`<li class="empty">${tr('scores.empty')}</li>`;
}
function recordScore(win,endingKey){
  const entry={win,character:selectedCharacter,distance:Math.max(0,Math.min(runLength,Math.round(18-player.position.z))),time:Math.max(0,Math.round(elapsed*1000)),date:Date.now(),endingKey,rescuedCows,collected:itemPickups.filter(i=>i.userData.collected).length+collectedMemoryThisRun,choice:finalChoice};entry.score=gameCore.rank_score(win?1:0,entry.distance,entry.time)+(entry.rescuedCows||0)*1000;
  const scoreOf=s=>Number.isFinite(s.score)?s.score:gameCore.rank_score(s.win?1:0,s.distance,s.time);
  const scores=[...readScores(),entry].sort((a,b)=>scoreOf(b)-scoreOf(a)).slice(0,8);
...
function makeSafeZone(index){
  ...
  group.visible=false;group.userData={index,residents,light,floorMat,fenceBoxes,house:houseData,campfire:{group:campfire,flames,embers,glow:fireGlow,light:fireLight,life:1},entered:false,passed:false,broken:false,timer:0,nextTalk:0};scene.add(group);return group;
}
const safeZones=Array.from({length:4},(_,i)=>makeSafeZone(i));
function obstacleOverlapsSanctuary(obstacle,zone,padding=0){
  const radius=obstacle.userData.collisionRadius||2.1,dx=obstacle.position.x-zone.position.x,dz=obstacle.position.z-zone.position.z;
  return Math.abs(dx)<24+radius+padding&&Math.abs(dz)<24+radius+padding;
}
function clearSanctuaryObstacles(){
  // 随机石头、树和刻纹物原本按整条世界坐标生成，偶尔会正好刷进后续才
  // 定位的安全屋，形成截图里的“一排黑石”。安全区整体连同门前通道必须净空。
  obstacles.forEach(obstacle=>{if(!obstacle.visible||!safeZones.some(zone=>obstacleOverlapsSanctuary(obstacle,zone,2)))return;obstacle.userData.visibleBeforeSanctuary=obstacle.visible;obstacle.userData.sanctuaryHidden=true;obstacle.userData.collisionDisabled=true;obstacle.visible=false;});
}
...
const witnessBubble=document.createElement('div'),superCowBubble=document.createElement('div'),enemySpeechBubble=document.createElement('div');
witnessBubble.className='npc-bubble witness-bubble';superCowBubble.className='npc-bubble super-cow-bubble';enemySpeechBubble.className='npc-bubble enemy-speech-bubble';witnessBubble.hidden=true;superCowBubble.hidden=true;enemySpeechBubble.hidden=true;npcBubbles.append(witnessBubble,superCowBubble,enemySpeechBubble);
let witnessSpeaker=null,witnessSpeechUntil=0,superCowSpeaker=null,superCowSpeechUntil=0,enemySpeechSpeaker=null,enemySpeechUntil=0,enemySpeechKey='';
const enemySpeechTimers=new Set();
...
function showEnemySpeech(speaker,key,duration=3300){if(!speaker||state!=='playing'||!speaker.visible)return;enemySpeechSpeaker=speaker;enemySpeechKey=key;enemySpeechUntil=elapsed+duration/1000;enemySpeechBubble.textContent=tr(key);enemySpeechBubble.hidden=false;sound(69,.34,'sawtooth',.055);}
function scheduleEnemySpeech(speaker,key,delay=0,duration=3300){const timer=setTimeout(()=>{enemySpeechTimers.delete(timer);showEnemySpeech(speaker,key,duration);},delay);enemySpeechTimers.add(timer);}
function enemyTaunt(enemy,key=`taunt.${enemy?.userData?.type||'beast'}`,delay=0,duration=3300){if(!enemy||enemy.userData.taunted)return false;enemy.userData.taunted=true;scheduleEnemySpeech(enemy,key,delay,duration);return true;}
function clearEnemySpeech(){enemySpeechTimers.forEach(clearTimeout);enemySpeechTimers.clear();enemySpeechSpeaker=null;enemySpeechKey='';enemySpeechUntil=0;enemySpeechBubble.hidden=true;}
function updateWorldSpeech(){
  for(const speech of [{speaker:witnessSpeaker,until:witnessSpeechUntil,bubble:witnessBubble,height:4},{speaker:superCowSpeaker,until:superCowSpeechUntil,bubble:superCowBubble,height:5.8},{speaker:enemySpeechSpeaker,until:enemySpeechUntil,bubble:enemySpeechBubble,height:7.1}]){
    ...
    const enemyBubble=bubble===enemySpeechBubble,halfWidth=Math.min(.46,Math.max(.08,(bubble.offsetWidth*.5+12)/width));bubble.style.left=(THREE.MathUtils.clamp(v.x*.5+.5,halfWidth,1-halfWidth)*width)+'px';bubble.style.top=(THREE.MathUtils.clamp(-v.y*.5+.5,enemyBubble?.17:.12,.82)*heightPx)+'px';bubble.hidden=v.z>1||(!enemyBubble&&(Math.abs(v.x)>1.35||Math.abs(v.y)>1.35));
  }
}
...
function makeHideGrass(){const g=new THREE.Group(),mat=flat(0x263f2c);for(let i=0;i<28;i++){const a=i/28*Math.PI*2,r=2.3+Math.random()*1.6,blade=mesh(new THREE.ConeGeometry(.18+Math.random()*.13,3.2+Math.random()*2,4),mat,g,Math.cos(a)*r,1.5,Math.sin(a)*r);blade.rotation.z=(Math.random()-.5)*.3;}g.userData.kind='grass';scene.add(g);return g;}
function makeAbandonedCar(){const g=new THREE.Group(),rust=flat(0x513129),dark=flat(0x121615);mesh(new THREE.BoxGeometry(7.4,2.2,4.2),rust,g,0,1.6,0);mesh(new THREE.BoxGeometry(4.1,1.5,3.5),rust,g,0,3,0);mesh(new THREE.BoxGeometry(3.3,1.05,.12),dark,g,0,3.15,-1.82);for(const x of [-3,3])for(const z of [-1.5,1.5])mesh(new THREE.CylinderGeometry(.75,.75,.5,8),dark,g,x,.65,z,[1,1,1],[0,0,Math.PI/2]);g.userData.kind='car';scene.add(g);return g;}
const hideSpots=[...Array.from({length:4},()=>({object:makeHideGrass(),kind:'grass',radius:4.3,limit:7,colliders:[]})),...Array.from({length:2},()=>{const object=makeAbandonedCar(),colliders=[-1,1].map(side=>{const proxy=new THREE.Object3D();proxy.userData.collisionRadius=1.6;scene.add(proxy);obstacles.push(proxy);return proxy;});return{object,kind:'car',radius:5.2,limit:11,colliders};})];
let currentHideSpot=null,hideTime=0,hideDiscovered=false;
...
const memoryPickups=MEMORY_IDS.slice(0,3).map((_,i)=>{const g=new THREE.Group(),core=mesh(new THREE.IcosahedronGeometry(.72,1),new THREE.MeshBasicMaterial({color:0xffd459,fog:false}),g,0,1.15,0),ring=mesh(new THREE.TorusGeometry(1.18,.1,6,20),new THREE.MeshBasicMaterial({color:0xff8d36,fog:false}),g,0,1.15,0,[1,1,1],[Math.PI/2,0,0]),light=new THREE.PointLight(0xffa83f,55,24);light.position.y=2;g.add(light);g.userData={memoryId:null,collected:false,core,ring,light};scene.add(g);return g;});
let collectedMemoryThisRun=0;
// 道具粒子在开局时一次性建立对象池。过去拾取瞬间才创建几十套几何体、
// 材质与 GPU 管线，低配手机第一次拿道具时会明显卡顿。
const pickupEffects=[],pickupParticleGeometry=new THREE.DodecahedronGeometry(.18,0),pickupRingGeometry=new THREE.RingGeometry(.8,1.18,24);
const pickupEffectPool=Array.from({length:4},()=>{const group=new THREE.Group(),particles=[],mat=new THREE.MeshBasicMaterial({color:0xffffff,transparent:true,opacity:0,fog:false,blending:THREE.AdditiveBlending});for(let i=0;i<quality.particles;i++){const particle=mesh(pickupParticleGeometry,mat,group,0,1.2,0);particles.push(particle);}const ringMat=new THREE.MeshBasicMaterial({color:0xffffff,transparent:true,opacity:0,side:THREE.DoubleSide,fog:false,blending:THREE.AdditiveBlending}),ring=mesh(pickupRingGeometry,ringMat,group,0,.18,0,[1,1,1],[-Math.PI/2,0,0]),light=new THREE.PointLight(0xffffff,0,34);light.position.y=2;group.add(light);group.visible=false;scene.add(group);return{group,particles,ring,light,mat,ringMat,age:0,active:false};});
function spawnPickupParticles(item){let effect=pickupEffectPool.find(candidate=>!candidate.active);if(!effect){effect=pickupEffects.shift();effect.active=false;}const color=itemColors[item.userData.type]||0xffffff;effect.active=true;effect.age=0;effect.group.visible=true;effect.group.position.copy(item.position);effect.group.scale.setScalar(1);effect.mat.color.setHex(color);effect.mat.opacity=1;effect.ringMat.color.setHex(color);effect.ringMat.opacity=1;effect.ring.scale.setScalar(1);effect.light.color.setHex(color);effect.light.intensity=95;effect.particles.forEach(particle=>{const angle=Math.random()*Math.PI*2,speed=5+Math.random()*11,scale=.62+Math.random()*1.45;particle.position.set(0,1.2,0);particle.rotation.set(0,0,0);particle.scale.setScalar(scale);particle.userData.velocity=new THREE.Vector3(Math.cos(angle)*speed,3+Math.random()*10,Math.sin(angle)*speed);particle.userData.spin=(Math.random()-.5)*14;});pickupEffects.push(effect);}
function resetPickupEffects(){pickupEffects.splice(0).forEach(effect=>{effect.active=false;effect.group.visible=false;effect.age=0;});}
let pickupEffectsPrewarmed=false;
function prewarmPickupEffects(){if(pickupEffectsPrewarmed)return;const effect=pickupEffectPool[0],visible=effect.group.visible,scale=effect.group.scale.clone();effect.group.visible=true;effect.group.scale.setScalar(.0001);effect.mat.opacity=.001;effect.ringMat.opacity=.001;renderer.compile(scene,camera);renderer.setScissorTest(true);renderer.setScissor(0,0,1,1);renderer.render(scene,camera);renderer.setScissorTest(false);effect.group.visible=visible;effect.group.scale.copy(scale);effect.mat.opacity=0;effect.ringMat.opacity=0;pickupEffectsPrewarmed=true;}
...
let proceduralLayoutSignature='';
function configureProceduralRoute(){const templates=['left','right','zigzag','split','narrow'],order=[];forkWalls.forEach((fork,i)=>{const template=templates[Math.floor(Math.random()*templates.length)],side=template==='left'?-1:template==='right'?1:(i%2?1:-1),x=template==='split'?0:side*(7+Math.random()*8),ratio=.225+i*.075+(Math.random()-.5)*.012;fork.position.set(x,0,18-runLength*ratio);fork.rotation.y=template==='zigzag'?(i%2?-.1:.1):(Math.random()-.5)*.05;fork.userData.template=template;fork.userData.openLane=side>0?'left':'right';fork.visible=true;order.push(`${template}:${x.toFixed(1)}:${ratio.toFixed(3)}`);});proceduralLayoutSignature=order.join('|');}
...
function resolvePlayerDynamicSolids(){
  ...
  for(const zone of safeZones){if(!zone.visible)continue;for(const box of zone.userData.fenceBoxes)pushPlayerFromBox(zone.position.x+box.x,zone.position.z+box.z,box.halfW,box.halfD);const house=zone.userData.house,hx=zone.position.x+house.x,hz=zone.position.z+house.z;pushPlayerFromBox(hx-house.halfW+.35,hz,.35,house.halfD);pushPlayerFromBox(hx+house.halfW-.35,hz,.35,house.halfD);pushPlayerFromBox(hx,hz+house.halfD-.35,house.halfW,.35);pushPlayerFromBox(hx-(house.halfW+house.doorHalf)/2,hz-house.halfD+.35,(house.halfW-house.doorHalf)/2,.35);pushPlayerFromBox(hx+(house.halfW+house.doorHalf)/2,hz-house.halfD+.35,(house.halfW-house.doorHalf)/2,.35);pushPlayerFromPoint(zone.position.x+5,zone.position.z+1,3.2);}
...
function resolveAnimalWorldCollision(animal,radius=1.25){
  ...
  for(const zone of safeZones){if(!zone.visible)continue;for(const box of zone.userData.fenceBoxes){const cx=zone.position.x+box.x,cz=zone.position.z+box.z,dx=collisionPoint.x-cx,dz=collisionPoint.z-cz,overlapX=box.halfW+radius-Math.abs(dx),overlapZ=box.halfD+radius-Math.abs(dz);if(overlapX<=0||overlapZ<=0)continue;if(overlapX<overlapZ){collisionPoint.x=cx+(dx>=0?box.halfW+radius:-box.halfW-radius);animal.userData.avoidSide=dx>=0?1:-1;}else collisionPoint.z=cz+(dz>=0?box.halfD+radius:-box.halfD-radius);hit=true;}const lx=collisionPoint.x-zone.position.x,lz=collisionPoint.z-zone.position.z,hx=lx+11,hz=lz-1,houseRadius=8.15,hd2=hx*hx+hz*hz;if(hd2<houseRadius*houseRadius){animal.userData.avoidSide=Math.sign(hx)||(animal.id%2?1:-1);if(hd2<.0001)collisionPoint.x+=houseRadius;else{const hd=Math.sqrt(hd2),push=houseRadius-hd;collisionPoint.x+=hx/hd*push;collisionPoint.z+=hz/hd*push;}hit=true;}}
...
function makeHostileMarkTexture(type){if(hostileMarkTextures.has(type))return hostileMarkTextures.get(type);const style=hostileMarkStyles[type]||hostileMarkStyles.beast,canvas=document.createElement('canvas');canvas.width=128;canvas.height=96;const ctx=canvas.getContext('2d'),stroke=(points,width=8,accent=false)=>{ctx.beginPath();points.forEach(([x,y],i)=>i?ctx.lineTo(x,y):ctx.moveTo(x,y));ctx.strokeStyle=accent?style.accent:style.color;ctx.lineWidth=width;ctx.lineCap='round';ctx.lineJoin='round';ctx.stroke();};ctx.clearRect(0,0,128,96);ctx.shadowColor=style.color;ctx.shadowBlur=11;
  ...
  else if(type==='slitWoman'){stroke([[22,47],[48,61],[64,55],[80,61],[106,47]],8);stroke([[27,48],[50,55],[64,51],[78,55],[101,48]],3,true);stroke([[64,18],[64,39]],5);}
...
function addHostileMark(enemy,index=0){const type=hostileMarkType(enemy),style=hostileMarkStyles[type]||hostileMarkStyles.beast,marker=new THREE.Group(),height=type==='tree'?14:type==='car'?7:type==='snake'?2.8:type==='toshio'?4.8:type==='kayako'?3.1:type==='slitWoman'?7.2:type==='jiangchen'?8.2:6.2,material=new THREE.SpriteMaterial({map:makeHostileMarkTexture(type),transparent:true,opacity:.58,depthWrite:false,depthTest:false,fog:false}),sprite=new THREE.Sprite(material);sprite.position.y=height;sprite.scale.set(style.scale[0],style.scale[1],1);sprite.renderOrder=999;marker.add(sprite);marker.userData={phase:index*.61,materials:[material],sprite,baseScale:style.scale,type};enemy.add(marker);hostileMarks.push(marker);return marker;}
...
// 都市传说原型使用原创“裂笑口罩客”造型：初见时戴着口罩，问完问题后
// 才露出不自然地延伸到两颊的嘴，并举起剪刀追赶。
function makeSlitWoman(){const g=new THREE.Group(),coat=flat(0x6b1728),skin=flat(0xc8b3aa),hair=new THREE.MeshBasicMaterial({color:0x10090d,fog:true}),maskMat=new THREE.MeshStandardMaterial({color:0xe5ded5,roughness:.88,flatShading:true}),mouthMat=new THREE.MeshBasicMaterial({color:0x52030a,fog:false}),tooth=flat(0xeee4d2),steel=new THREE.MeshStandardMaterial({color:0xc8ccd0,metalness:.82,roughness:.24}),eye=new THREE.MeshBasicMaterial({color:0x16070a,fog:false});mesh(new THREE.ConeGeometry(1.02,4.9,7),coat,g,0,2.45,0);const head=mesh(new THREE.SphereGeometry(.7,8,6),skin,g,0,5.35,-.08,[.9,1.08,.78]);mesh(new THREE.SphereGeometry(.82,7,5),hair,head,0,.34,.2,[1.08,1.18,.88]);for(const x of [-.25,.25])mesh(new THREE.SphereGeometry(.075,6,5),eye,head,x,.08,-.63,[1.3,.62,.38]);const mask=mesh(new THREE.BoxGeometry(1.12,.58,.15),maskMat,head,0,-.24,-.69,[1,1,.55]);const slitParts=[];for(const side of [-1,1]){const cut=mesh(new THREE.CapsuleGeometry(.055,.86,2,5),mouthMat,head,side*.42,-.28,-.71,[1,1,1],[0,0,side*1.02]);slitParts.push(cut);for(let i=0;i<3;i++)slitParts.push(mesh(new THREE.ConeGeometry(.035,.16,4),tooth,head,side*(.14+i*.16),-.26,-.755,[1,1,1],[side>0?Math.PI:0,0,0]));}slitParts.forEach(part=>part.visible=false);const arms=[],legs=[];for(const x of [-1,1])arms.push(mesh(new THREE.CapsuleGeometry(.14,1.72,2,5),coat,g,x*.82,3.05,0,[1,1,1],[0,0,x*.15]));for(const x of [-.3,.3])legs.push(mesh(new THREE.CapsuleGeometry(.18,1.45,2,5),coat,g,x,.78,0));const scissors=new THREE.Group();for(const side of [-1,1]){mesh(new THREE.TorusGeometry(.18,.045,5,10),steel,scissors,side*.2,.12,0,[1,1,1],[Math.PI/2,0,0]);mesh(new THREE.CapsuleGeometry(.035,.95,2,4),steel,scissors,side*.13,-.46,0,[1,1,1],[0,0,side*.13]);}scissors.position.set(0,-1.05,-.15);arms[1].add(scissors);g.userData={type:'slitWoman',enemyKey:'enemy.slitWoman',arms,legs,mask,slitParts,scissors,chaseSpeed:7.05,specialGhost:true,revealed:false};g.visible=false;scene.add(g);return g;}
...
const sadako=makeSadako(),kayako=makeKayako(),toshio=makeToshio(),slitWoman=makeSlitWoman(),kayakoStairs=makeGhostStairs(),cursedTelevision=makeCursedTelevision();[sadako,kayako,toshio,slitWoman].forEach((ghost,i)=>addHostileMark(ghost,allEnemies.length+1+i));
let toshioStage=0,kayakoStage=0,sadakoStage=0,slitStage=0,toshioStageAt=0,kayakoStageAt=0,sadakoStageAt=0,slitStageAt=0,toshioRatios=[],kayakoRatios=[],sadakoRatios=[],slitRatios=[],toshioIndex=0,kayakoIndex=0,sadakoIndex=0,slitIndex=0;
...
function updateObjectives(progress,dt,t,moving,safeProtected){
  ...
  sheltered=false;currentHideSpot=null;
  shelterHuts.forEach(hut=>{hut.userData.warm.visible=!performanceMode||Math.abs(player.position.z-hut.position.z)<90;if(near(hut,6.2)&&!moving){sheltered=true;currentHideSpot={object:hut,kind:'hut',limit:14};stamina=Math.min(100,stamina+dt*30);}});
  if(!currentHideSpot&&!moving){currentHideSpot=hideSpots.find(spot=>spot.object.visible&&near(spot.object,spot.radius))||null;if(currentHideSpot)sheltered=true;}
  if(currentHideSpot){hideTime+=dt;if(hideTime>currentHideSpot.limit&&!hideDiscovered){hideDiscovered=true;terrorFlash();say('hide.discovered',2600);}if(!hideDiscovered){missionKey='hide.hidden';missionParams={seconds:Math.max(0,Math.ceil(currentHideSpot.limit-hideTime))};stamina=Math.min(100,stamina+dt*24);}else{missionKey='hide.discovered';}}
  else{hideTime=0;hideDiscovered=false;}
  document.body.classList.toggle('sheltered',sheltered&&!hideDiscovered);
  if(currentChapter===2&&!sheltered&&progress>runLength*.43&&progress<runLength*.58){missionKey='task.shelterMission';}
  if(sheltered&&!hideDiscovered&&!missionKey){missionKey='task.sheltered';}

  memoryPickups.forEach(item=>{if(item.userData.collected||!item.visible)return;item.rotation.y+=dt*1.4;item.userData.ring.rotation.z+=dt*2.3;item.position.y=.05+Math.sin(t*3+item.id)*.18;if(near(item,3)){item.userData.collected=true;item.visible=false;collectedMemoryThisRun++;collectMemory(item.userData.memoryId);spawnPickupParticles({position:item.position,userData:{type:'health'}});playPickupSound('health');}});
  ...
  if(progress>runLength*.805){
    if(!choicePrompted){choicePrompted=true;finalChoice='pending';finalChoiceEl.classList.add('show');finalChoiceEl.setAttribute('aria-hidden','false');document.body.classList.add('choice-open');Object.keys(keys).forEach(key=>delete keys[key]);resetJoystick();say('choice.prompt',3000);requestAnimationFrame(()=>document.querySelector('#rescueChoiceBtn').focus({preventScroll:true}));}
    if(finalChoice&&finalChoice!=='pending'&&!finalWaveStarted){finalWaveStarted=true;say(finalChoice==='rescue'?'task.stampede':'choice.aloneChosen',3300);terrorFlash();const mode=difficulties[selectedCharacter],desired=finalChoice==='rescue'?mode.stalkers:Math.max(2,Math.floor(mode.stalkers*.42)),remaining=Math.max(0,desired-stalkers.filter(enemy=>enemy.userData.spawned).length);spawnStalkerWave(4,remaining);}
    if(finalChoice==='rescue')finalHerd.forEach((cow,i)=>{if(cow.userData.escaped||cow.userData.eaten)return;cow.visible=true;talkToPassingNpc(cow,i+12);if(holdNpcInSafeZone(cow,t,dt))return;cow.position.z-=dt*(i===0?11.8:6.4+(i%4)*.24);cow.position.x+=(cow.userData.escapeX||0)*dt+Math.sin(t*1.8+i)*dt*(i===0?.75:.38);animateCow(cow,t,i===0?17:11);if(cow.userData.superCape){cow.userData.superCape.rotation.x=Math.sin(t*12)*.12;cow.userData.superCape.position.y=cow.userData.superCape.userData.baseY+Math.sin(t*9)*.09;}if(cow.position.z<exitZ+4){cow.userData.escaped=true;rescuedCows++;cow.visible=false;}});
    if(finalChoice==='pending'){missionKey='choice.mission';}
  }
  return {protectedZone:safeProtected||(sheltered&&!hideDiscovered),missionKey,missionParams};
}
...
function talkToPassingNpc(npc,index){
  if(npc.userData.passingTalked||npc.userData.eaten||npc.userData.escaped||elapsed-lastPassingNpcTalk<3.15)return false;
  npc.getWorldPosition(collisionPoint);if(Math.hypot(player.position.x-collisionPoint.x,player.position.z-collisionPoint.z)>27)return false;
  const key=passingNpcLines[index%passingNpcLines.length];npc.userData.passingTalked=true;lastPassingNpcTalk=elapsed;say(key,4000);showWorldSpeech(npc,key,4200);sound(176+(index%4)*18,.16,'triangle',.034);return true;
}
function talkToRoadNpc(npc,index){
  if(npc.userData.talked||npc.userData.eaten)return false;
  const distance=Math.hypot(player.position.x-npc.position.x,player.position.z-npc.position.z);if(distance>=30)return false;
  const key=`roadNpc.${index}`;npc.userData.talked=true;lastPassingNpcTalk=elapsed;say(key,4200);showWorldSpeech(npc,key,4400);sound(164+index*22,.17,'triangle',.036);return true;
}
...
function playGhostSignature(type){
  ...
  if(type==='slitWoman'){[1480,620,1320,410].forEach((freq,i)=>setTimeout(()=>sound(freq,.11,i%2?'square':'triangle',.072),i*135));return;}
...
function unlockAudio(){
  ...
  prepareFingerAudio();document.querySelector('#audioGate').classList.add('hidden');
  playTitleCry();
}
...
function start(){
  ...
  resetPickupEffects();
  knockedObstacles.splice(0);obstacles.forEach(obstacle=>{const original=obstacle.userData.originalTransform;if(original){obstacle.position.copy(original.position);obstacle.rotation.copy(original.rotation);obstacle.visible=original.visible;}if(obstacle.userData.sanctuaryHidden){obstacle.visible=obstacle.userData.visibleBeforeSanctuary;delete obstacle.userData.sanctuaryHidden;delete obstacle.userData.visibleBeforeSanctuary;}obstacle.userData.collisionDisabled=false;obstacle.userData.knockVelocity=null;});
  ...
  toshioStage=0;kayakoStage=0;sadakoStage=0;slitStage=0;toshioStageAt=0;kayakoStageAt=0;sadakoStageAt=0;slitStageAt=0;toshioIndex=0;kayakoIndex=0;sadakoIndex=0;slitIndex=0;[toshio,kayako,sadako,slitWoman].forEach(ghost=>{ghost.visible=false;ghost.position.y=.05;ghost.rotation.set(0,0,0);ghost.scale.setScalar(1);ghost.userData.pounce=null;ghost.userData.stunnedUntil=0;ghost.userData.feeding=0;ghost.userData.taunted=false;});slitWoman.userData.revealed=false;slitWoman.userData.mask.visible=true;slitWoman.userData.slitParts.forEach(part=>part.visible=false);kayakoStairs.visible=false;cursedTelevision.visible=false;cursedTelevision.userData.screenMat.opacity=.82;sadako.userData.nextTeleport=0;
  // 特殊怪物严格按章节累积解锁：第一章只出现黑眼童影，之后每章加入新类型，已解锁者仍会再次出现。
  const encounterSets=selectedCharacter==='orange'?{toshio:[.12,.31,.57,.83],kayako:[.27,.52,.88],sadako:[.44,.73],slit:[.63,.91],freddy:[.68],edward:[.82],jiangchen:[.9]}:selectedCharacter==='yellow'?{toshio:[.1,.29,.48,.67,.86],kayako:[.24,.45,.7,.9],sadako:[.42,.61,.84],slit:[.62,.76,.93],freddy:[.66,.81,.95],edward:[.81,.92],jiangchen:[.86,.97]}:{toshio:[.09,.25,.43,.62,.81,.94],kayako:[.22,.4,.58,.76,.92],sadako:[.41,.56,.72,.9],slit:[.61,.7,.83,.95],freddy:[.64,.75,.87,.96],edward:[.8,.88,.96],jiangchen:[.84,.91,.98]};toshioRatios=encounterSets.toshio;kayakoRatios=encounterSets.kayako;sadakoRatios=encounterSets.sadako;slitRatios=encounterSets.slit;freddyRatios=encounterSets.freddy;edwardRatios=encounterSets.edward;jiangchenRatios=encounterSets.jiangchen;freddyIndex=0;edwardIndex=0;jiangchenIndex=0;freddyUntil=0;edwardStage=0;edwardStageAt=0;jiangchenUntil=0;[freddy,jiangchen,edwardRipper,dreamClock].forEach(actor=>{actor.visible=false;actor.position.y=.05;actor.rotation.set(0,0,0);actor.scale.setScalar(1);actor.userData.taunted=false;});freddy.userData.pounce=null;jiangchen.userData.pounce=null;
  clothCount=0;switchCount=0;sheltered=false;currentHideSpot=null;hideTime=0;hideDiscovered=false;flashlightUntil=0;speedBoostUntil=0;smokeCharges=0;radioOwned=false;fakeExitTriggered=false;finalWaveStarted=false;rescuedCows=0;finalChoice=null;choicePrompted=false;collectedMemoryThisRun=0;finalChoiceEl.classList.remove('show');finalChoiceEl.setAttribute('aria-hidden','true');npcShoveHintShown=false;lastNpcShove=-9;
  ...
  configureProceduralRoute();
  hideSpots.forEach((spot,i)=>{const ratio=.16+i*.12+(Math.random()-.5)*.02,side=i%2?-1:1;spot.object.position.set(side*(16+Math.random()*15),0,18-runLength*ratio);spot.object.rotation.y=(Math.random()-.5)*.5;spot.object.visible=true;spot.colliders.forEach((proxy,n)=>{proxy.position.set(spot.object.position.x+(n?3.1:-3.1),0,spot.object.position.z);proxy.visible=true;});});
...
function chooseEnding(win){
  ...
  if(win&&finalChoice==='rescue'&&rescuedCows>=7)return'ending.herd';
  if(win&&finalChoice==='rescue')return'ending.rescueFailed';
...
document.querySelectorAll('.character').forEach(button=>button.addEventListener('click',()=>{
  ...
  if(button.dataset.character==='super'&&persistentProgress.memories.length<6){say('character.superLocked',2200);return;}
...
let fingerWhooshBuffer=null,fingerAudioPrepared=false;
function prepareFingerAudio(){
  if(!audio||fingerAudioPrepared)return;fingerWhooshBuffer=audio.createBuffer(1,Math.max(1,Math.floor(audio.sampleRate*.16)),audio.sampleRate);const data=fingerWhooshBuffer.getChannelData(0);for(let i=0;i<data.length;i++)data[i]=(Math.random()*2-1)*(1-i/data.length);const source=audio.createBufferSource(),filter=audio.createBiquadFilter(),gain=audio.createGain();source.buffer=audio.createBuffer(1,1,audio.sampleRate);filter.type='bandpass';gain.gain.value=0;source.connect(filter).connect(gain).connect(audio.destination);source.start();fingerAudioPrepared=true;
}
function prewarmShoveVisual(character){
  const hands=character?.userData?.shoveFinger?.hands;if(!hands?.length||character.userData.shovePrewarmed)return;const states=hands.map(hand=>({visible:hand.rig.visible,scale:hand.rig.scale.clone()}));hands.forEach(hand=>{hand.rig.visible=true;hand.rig.scale.setScalar(.0001);});renderer.compile(scene,camera);renderer.setScissorTest(true);renderer.setScissor(0,0,1,1);renderer.render(scene,camera);renderer.setScissorTest(false);hands.forEach((hand,i)=>{hand.rig.visible=states[i].visible;hand.rig.scale.copy(states[i].scale);});character.userData.shovePrewarmed=true;
}
function playFingerWhoosh(){
  if(!audio)audio=new (window.AudioContext||window.webkitAudioContext)();audio.resume?.().catch(()=>{});prepareFingerAudio();const now=audio.currentTime,source=audio.createBufferSource(),filter=audio.createBiquadFilter(),gain=audio.createGain();source.buffer=fingerWhooshBuffer;filter.type='bandpass';filter.frequency.setValueAtTime(1450,now);filter.frequency.exponentialRampToValueAtTime(310,now+.16);filter.Q.value=.7;gain.gain.setValueAtTime(.0001,now);gain.gain.exponentialRampToValueAtTime(.115,now+.018);gain.gain.exponentialRampToValueAtTime(.0001,now+.16);source.connect(filter).connect(gain).connect(audio.destination);source.start(now);
}
...
let specialEntranceTimer=0;const specialEntranceClasses=['entrance-cat','entrance-crawler','entrance-well','entrance-dream','entrance-mistblade','entrance-corpse','entrance-smile'];
...
function animateClassicGhost(enemy,t){
  ...
  else if(enemy.userData.type==='slitWoman'){enemy.position.y=.05+Math.abs(Math.sin(t*5.8))*.08;enemy.userData.legs?.forEach((leg,i)=>leg.rotation.x=Math.sin(t*8+i*Math.PI)*.42);enemy.userData.arms?.forEach((arm,i)=>arm.rotation.x=(enemy.userData.revealed?(i?-.9:-.38):-.12)+Math.sin(t*5+i*Math.PI)*.2);if(enemy.userData.scissors)enemy.userData.scissors.rotation.z=Math.sin(t*9)*.16;}
}
...
function updateClassicGhostSetpieces(progress,dt,t,safeProtected){
  ...
  if(slitStage===4&&slitIndex<slitRatios.length)slitStage=0;
  if(currentChapter>=0&&!safeProtected&&toshioStage===0&&toshioIndex<toshioRatios.length&&progress>runLength*toshioRatios[toshioIndex]){const appearance=toshioIndex++;toshioStage=1;toshioStageAt=elapsed;toshio.scale.setScalar(classicEncounterScale(appearance));toshio.position.set(THREE.MathUtils.clamp(player.position.x+(Math.random()<.5?-1:1)*(8+Math.random()*5),-42,42),.05,player.position.z-22);toshio.visible=true;specialEntrance('cat',1500);terrorFlash();playGhostSignature('toshio');say('ghost.toshioAppears',2900);enemyTaunt(toshio,'taunt.toshio',900,2800);}
  ...
  if(currentChapter>=1&&!safeProtected&&kayakoStage===0&&kayakoIndex<kayakoRatios.length&&progress>runLength*kayakoRatios[kayakoIndex]){const appearance=kayakoIndex++;kayakoStage=1;kayakoStageAt=elapsed;kayako.scale.setScalar(classicEncounterScale(appearance));kayako.position.set(0,.05,0);kayakoStairs.position.set(THREE.MathUtils.clamp(player.position.x+(Math.random()<.5?-1:1)*(11+Math.random()*6),-39,39),0,player.position.z-28);kayakoStairs.visible=true;kayako.visible=true;specialEntrance('crawler',2100);terrorFlash();playGhostSignature('kayako');say('ghost.kayakoStairs',3400);enemyTaunt(kayako,'taunt.kayako',1100,3000);}
  ...
  if(sadakoStage===2&&sadako.visible)enemyTaunt(sadako,'taunt.sadako',650,3000);

  // 裂笑口罩客有明确且可躲避的两段式出场：先停在前方问话，口罩落下后
  // 才进入追逐。预警期间不会攻击，玩家可以绕开或提前拉开距离。
  if(currentChapter>=3&&!safeProtected&&slitStage===0&&slitIndex<slitRatios.length&&progress>runLength*slitRatios[slitIndex]){const appearance=slitIndex++;slitStage=1;slitStageAt=elapsed;slitWoman.scale.setScalar(classicEncounterScale(appearance));slitWoman.position.set(THREE.MathUtils.clamp(player.position.x+(Math.random()<.5?-1:1)*(7+Math.random()*5),-40,40),.05,player.position.z-24);slitWoman.rotation.y=Math.atan2(slitWoman.position.x-player.position.x,slitWoman.position.z-player.position.z);slitWoman.userData.revealed=false;slitWoman.userData.mask.visible=true;slitWoman.userData.slitParts.forEach(part=>part.visible=false);slitWoman.userData.taunted=false;slitWoman.visible=true;specialEntrance('smile',1800);terrorFlash();playGhostSignature('slitWoman');showEnemySpeech(slitWoman,'ghost.slitQuestion',3000);}
  if(slitStage===1){slitWoman.rotation.y=Math.atan2(slitWoman.position.x-player.position.x,slitWoman.position.z-player.position.z);animateClassicGhost(slitWoman,t);if(elapsed-slitStageAt>2.55){slitStage=2;slitStageAt=elapsed;slitWoman.userData.revealed=true;slitWoman.userData.mask.visible=false;slitWoman.userData.slitParts.forEach(part=>part.visible=true);specialEntrance('smile',1350);terrorFlash();playGhostSignature('slitWoman');showEnemySpeech(slitWoman,'ghost.slitReveal',2750);}}
  if(slitStage===2&&elapsed-slitStageAt>1.15){slitStage=3;slitStageAt=elapsed;addClassicGhostChaser(slitWoman,7.05+(slitIndex-1)*.2);slitWoman.userData.attackReadyAt=elapsed+1.75;}
  if(slitStage===3){const d=Math.hypot(slitWoman.position.x-player.position.x,slitWoman.position.z-player.position.z),chaseLimit=selectedCharacter==='orange'?10:selectedCharacter==='yellow'?13:16;if((player.position.z<slitWoman.position.z&&d>55)||elapsed-slitStageAt>chaseLimit){slitStage=4;removeClassicGhostChaser(slitWoman);say('ghost.slitEscaped',2200);}}
...
function startFreddyEncounter(){const index=freddyIndex++;freddy.scale.setScalar(encounterScale('freddy',index));freddy.position.set(player.position.x+(Math.random()<.5?-1:1)*11,.05,player.position.z+27);freddy.userData.stunnedUntil=0;freddy.userData.taunted=false;addClassicGhostChaser(freddy,6.55+index*.22);freddyUntil=elapsed+13+(selectedCharacter==='leopard'?3:0);dreamClock.position.set(THREE.MathUtils.clamp(player.position.x+(Math.random()<.5?-1:1)*15,-45,45),.05,player.position.z-24);dreamClock.visible=true;document.body.classList.add('dreaming');specialEntrance('dream',1900);terrorFlash();playGhostSignature('sadako');say('ghost.freddyDream',3200);enemyTaunt(freddy,'taunt.freddy',900,3000);}
function startEdwardEncounter(){const index=edwardIndex++,scale=encounterScale('edward',index);edwardRipper.scale.setScalar(scale);edwardDirection=Math.random()<.5?-1:1;edwardRipper.position.set(edwardDirection*54,.05,player.position.z-17-Math.random()*9);edwardRipper.rotation.y=edwardDirection>0?Math.PI/2:-Math.PI/2;edwardRipper.visible=true;edwardRipper.userData.hitEncounter=false;edwardRipper.userData.taunted=false;edwardStage=1;edwardStageAt=elapsed;specialEntrance('mistblade',1700);sound(126,.55,'sawtooth',.07);say('ghost.edwardFog',2500);enemyTaunt(edwardRipper,'taunt.edward',700,2900);}
function finishEdward(){edwardRipper.visible=false;edwardStage=0;edwardRipper.userData.hitEncounter=false;}
function startJiangchenEncounter(){const index=jiangchenIndex++,scale=encounterScale('jiangchen',index);jiangchen.scale.setScalar(scale);jiangchen.position.set(player.position.x+(Math.random()<.5?-1:1)*16,.05,player.position.z+34);jiangchen.userData.stunnedUntil=0;jiangchen.userData.taunted=false;addClassicGhostChaser(jiangchen,7.05+index*.25);jiangchenUntil=elapsed+16+(selectedCharacter==='leopard'?4:0);specialEntrance('corpse',2100);terrorFlash();horrorSound(3);say('ghost.jiangchenLands',3200);enemyTaunt(jiangchen,'taunt.jiangchen',850,3000);}
function updateDistributedEncounters(progress,dt,t,safeProtected){
  if(currentChapter>=3&&!safeProtected&&freddyIndex<freddyRatios.length&&progress>runLength*freddyRatios[freddyIndex]&&!freddy.visible)startFreddyEncounter();
  ...
  if(currentChapter>=4&&!safeProtected&&edwardIndex<edwardRatios.length&&progress>runLength*edwardRatios[edwardIndex]&&edwardStage===0)startEdwardEncounter();
  ...
  if(currentChapter>=4&&!safeProtected&&jiangchenIndex<jiangchenRatios.length&&progress>runLength*jiangchenRatios[jiangchenIndex]&&!jiangchen.visible)startJiangchenEncounter();
...
function spawnStalkerWave(chapter,count){
  ...
  chosen.slice(0,2).forEach((enemy,i)=>enemyTaunt(enemy,`taunt.${enemy.userData.type}`,650+i*3500,3000));
...
function updateStoryWave(progress){
  ...
  if(storyStage===0&&(progress>18||elapsed>2.5)){
    storyStage=1;const pack=[[-12,36],[-7,31],[-2,39],[3,33],[8,40],[13,35]],members=wolfPack.slice(0,mode.pack);members.forEach((e,i)=>placeChaser(e,pack[i][0],pack[i][1],6.85+(i%3)*.16));members.slice(0,2).forEach((enemy,i)=>enemyTaunt(enemy,'taunt.beast',850+i*3400,2900));say('event.pack',2200,{count:mode.pack});glitch();
  }
  ...
  if(storyStage===3&&progress>runLength*.605){
    storyStage=4;spawnStalkerWave(3,mode.chapterWaves[2]);placeChaser(monsterCar,0,64,10.2);enemyTaunt(monsterCar,'taunt.car',900,3000);hunterGlow.visible=true;say('event.car',3200);sound(31,1.6,'sawtooth',.14);glitch();
  }
...
function tick(){
  ...
  if(state==='playing'&&finalChoice==='pending'){renderer.render(scene,camera);return;}
  ...
  if(state==='playing'){
    ...
    for(const snake of snakes){
      ...
      const d=Math.hypot(player.position.x-snake.position.x,player.position.z-snake.position.z);nearest=Math.min(nearest,d);if(!safeProtected&&d<28)enemyTaunt(snake,'taunt.snake',0,3000);if(!safeProtected&&d<4.2){stamina=Math.max(0,stamina-25*difficulty.hazard*dt);player.position.x+=(player.position.x-snake.position.x)*dt*2.5;shake=.28;damageHeart('event.snakeHit',snake,'death.snake');}
    }
    if(!safeProtected)for(const tree of treeEnemies){if(tree.userData.collisionDisabled)continue;tree.userData.arms.forEach((a,i)=>a.rotation.z+=(i?1:-1)*dt*.45);const d=Math.hypot(player.position.x-tree.position.x,player.position.z-tree.position.z);nearest=Math.min(nearest,d);if(d<30)enemyTaunt(tree,'taunt.tree',0,3000);if(d<5.2){stamina=Math.max(0,stamina-18*difficulty.hazard*dt);shake=.2;damageHeart('event.treeHit',tree,'death.tree');}}
...
if(import.meta.env.DEV)window.__NIULAI_TEST__={
  ...
  shovePrewarmState(){return{visual:Boolean(player.userData.shovePrewarmed),audio:fingerAudioPrepared,bufferFrames:fingerWhooshBuffer?.length||0};},
  ...
  slitEncounterProbe(){currentChapter=3;slitRatios=[0];slitIndex=0;slitStage=0;player.position.set(0,.05,-40);updateClassicGhostSetpieces(41,.016,clock.elapsedTime,false);return{stage:slitStage,visible:slitWoman.visible,mask:slitWoman.userData.mask.visible,slit:slitWoman.userData.slitParts.some(part=>part.visible),speech:enemySpeechBubble.textContent,attackReadyAt:slitWoman.userData.attackReadyAt||0};},
  advanceSlit(seconds=3){slitStageAt=elapsed-seconds;updateClassicGhostSetpieces(41,.016,clock.elapsedTime,false);return{stage:slitStage,visible:slitWoman.visible,mask:slitWoman.userData.mask.visible,slit:slitWoman.userData.slitParts.some(part=>part.visible),chasing:activeChasers.includes(slitWoman),speech:enemySpeechBubble.textContent,attackReadyAt:slitWoman.userData.attackReadyAt||0,elapsed};},
  ...
  ageHide(seconds){hideTime=seconds;return true;},
  hidingState(){return{kind:currentHideSpot?.kind||null,time:hideTime,discovered:hideDiscovered,sheltered};},
  choiceProbe(){player.position.set(0,.05,18-runLength*.82);choicePrompted=false;finalChoice=null;return true;},
  selectChoice(choice){selectFinalChoice(choice);return finalChoice;},
  choiceState(){return{choice:finalChoice,prompted:choicePrompted,visible:finalChoiceEl.classList.contains('show'),paused:document.body.classList.contains('choice-open'),elapsed,playerX:player.position.x,playerZ:player.position.z,rescueVisible:finalHerd.some(c=>c.visible)};},
  pickupProbe(type='speed'){const item=itemPickups.find(candidate=>candidate.userData.type===type)||itemPickups[0];item.userData.collected=false;item.visible=true;item.position.copy(player.position);return true;},
  pickupPoolState(){return{active:pickupEffects.length,pool:pickupEffectPool.length,sceneResident:pickupEffectPool.every(effect=>effect.group.parent===scene)};},
  specialSchedule(){return{toshio:toshioRatios,kayako:kayakoRatios,sadako:sadakoRatios,slit:slitRatios,freddy:freddyRatios,edward:edwardRatios,jiangchen:jiangchenRatios};},
  modelState(){return{kind:selectedCharacter,version:player.userData.modelVersion||null,parts:player.children.length};},
