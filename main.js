...
function start(){
  ...
  const availableMemories=MEMORY_IDS.filter(id=>!persistentProgress.memories.includes(id));memoryPickups.forEach((item,i)=>{const id=availableMemories[i]||MEMORY_IDS[(persistentProgress.runs+i)%MEMORY_IDS.length];item.userData.memoryId=id;item.userData.collected=false;item.position.set((i%2?1:-1)*(10+Math.random()*18),.05,18-runLength*(.19+i*.29));item.visible=true;});persistentProgress.runs++;saveProgress();
