...
function updateObjectives(progress,dt,t,moving,safeProtected){
  ...
  if(progress>runLength*.805){
    if(!choicePrompted){choicePrompted=true;finalChoice='pending';finalChoiceEl.classList.add('show');finalChoiceEl.setAttribute('aria-hidden','false');say('choice.prompt',3000);}
...
function start(){
  ...
  const oldPlayer=player;player=createCharacter(selectedCharacter);player.position.set(0,.05,18);player.rotation.set(0,0,0);scene.remove(oldPlayer);scene.add(player);renderHearts();prewarmShoveVisual(player);prepareFingerAudio();storyStage=0;waveWarningStage=0;activeChasers=[];allEnemies.forEach(e=>{e.visible=false;e.position.y=e.userData.type==='car'?0:.05;e.userData.feeding=0;e.userData.joined=false;e.userData.headBob=0;e.userData.spawned=false;e.userData.pounce=null;e.userData.nextPounce=0;e.userData.stunnedUntil=0;e.userData.taunted=false;});scalableEnemies.forEach(enemy=>{enemy.userData.sizeMultiplier=1;enemy.scale.copy(enemy.userData.baseScale);});stalkers.forEach(e=>e.position.copy(e.userData.home));monsterCar.position.set(0,0,80);snakes.forEach(snake=>{snake.position.copy(snake.userData.home);snake.rotation.set(0,Math.PI/2,0);snake.userData.avoidTimer=0;snake.visible=true;});treeEnemies.forEach(e=>e.visible=true);snowGhost.userData.stunnedUntil=0;snowGhost.userData.taunted=false;
...
function selectFinalChoice(choice){if(state!=='playing'||finalChoice!=='pending')return;finalChoice=choice;finalChoiceEl.classList.remove('show');finalChoiceEl.setAttribute('aria-hidden','true');say(choice==='rescue'?'choice.rescueChosen':'choice.aloneChosen',2600);sound(choice==='rescue'?74:132,.55,'sawtooth',.07);}
...
if(import.meta.env.DEV)window.__NIULAI_TEST__={
  ...
  choiceState(){return{choice:finalChoice,prompted:choicePrompted,visible:finalChoiceEl.classList.contains('show'),rescueVisible:finalHerd.some(c=>c.visible)};},
