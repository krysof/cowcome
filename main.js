...
function updateClassicGhostSetpieces(progress,dt,t,safeProtected){
  ...
  if(kayakoStage===2&&elapsed-kayakoStageAt>8){const d=Math.hypot(kayako.position.x-player.position.x,kayako.position.z-player.position.z);if(player.position.z<kayako.position.z&&d>58){kayakoStage=3;removeClassicGhostChaser(kayako);say('ghost.kayakoEscaped',2200);}}
...
function startFreddyEncounter(){const index=freddyIndex++;freddy.scale.setScalar(encounterScale('freddy',index));freddy.position.set(player.position.x+(Math.random()<.5?-1:1)*11,.05,player.position.z+27);freddy.userData.stunnedUntil=0;addClassicGhostChaser(freddy,6.55+index*.22);freddyUntil=elapsed+13+(selectedCharacter==='leopard'?3:0);dreamClock.position.set(THREE.MathUtils.clamp(player.position.x+(Math.random()<.5?-1:1)*15,-45,45),.05,player.position.z-24);dreamClock.visible=true;document.body.classList.add('dreaming');terrorFlash();playGhostSignature('sadako');say('ghost.freddyDream',3200);}
function startEdwardEncounter(){const index=edwardIndex++,scale=encounterScale('edward',index);edwardRipper.scale.setScalar(scale);edwardDirection=Math.random()<.5?-1:1;edwardRipper.position.set(edwardDirection*54,.05,player.position.z-17-Math.random()*9);edwardRipper.rotation.y=edwardDirection>0?Math.PI/2:-Math.PI/2;edwardRipper.visible=true;edwardRipper.userData.hitEncounter=false;edwardStage=1;edwardStageAt=elapsed;sound(126,.55,'sawtooth',.07);say('ghost.edwardFog',2500);}
function finishEdward(){edwardRipper.visible=false;edwardStage=0;edwardRipper.userData.hitEncounter=false;}
function startJiangchenEncounter(){const index=jiangchenIndex++,scale=encounterScale('jiangchen',index);jiangchen.scale.setScalar(scale);jiangchen.position.set(player.position.x+(Math.random()<.5?-1:1)*16,.05,player.position.z+34);jiangchen.userData.stunnedUntil=0;addClassicGhostChaser(jiangchen,7.05+index*.25);jiangchenUntil=elapsed+16+(selectedCharacter==='leopard'?4:0);terrorFlash();horrorSound(3);say('ghost.jiangchenLands',3200);}
