...
function addShoveFinger(character,skin,x=.9,y=2.55){
  const hands=[-1,1].map(side=>{
    ...
    // 不是一根圆柱：掌心、蜷起的四指、拇指、三段食指、指甲和关节纹共同组成真正可辨认的手。
    ...
    const segments=[mesh(new THREE.CapsuleGeometry(.15,.5,3,8),skin,finger,0,.36,0,[1,1,1]),mesh(new THREE.CapsuleGeometry(.135,.46,3,8),skin,finger,0,.91,0,[.96,1,.96]),mesh(new THREE.CapsuleGeometry(.122,.38,3,8),skin,finger,0,1.39,0,[.92,1,.92])];
    const tip=mesh(new THREE.SphereGeometry(.13,9,7),skin,finger,0,1.68,0,[.92,1.08,.95]);
    const nailMat=new THREE.MeshStandardMaterial({color:0xffe7da,roughness:.58,metalness:0,flatShading:false}),nail=mesh(new THREE.SphereGeometry(.085,9,6),nailMat,finger,0,1.72,-.085,[.72,1.08,.22]);
    ...
    const basePosition=rig.position.clone(),setExtension=amount=>{amount=THREE.MathUtils.clamp(amount,0,1);segments[0].scale.y=1+amount*.42;segments[1].scale.y=1+amount*.48;segments[1].position.y=.91+amount*.34;segments[2].scale.y=1+amount*.52;segments[2].position.y=1.39+amount*.72;tip.position.y=1.68+amount*1.08;nail.position.y=1.72+amount*1.08;creases[0].position.y=.68+amount*.16;creases[1].position.y=1.18+amount*.54;finger.userData.extension=amount;};
...
const pickupEffects=[],itemColors={herb:0x65ff51,flashlight:0xfff27c,radio:0x58cfff,smoke:0xe5e5e5,health:0xff2446};
...
let toshioStage=0,kayakoStage=0,sadakoStage=0,toshioStageAt=0,kayakoStageAt=0,sadakoStageAt=0;
...
function start(){
  ...
  toshioStage=0;kayakoStage=0;sadakoStage=0;toshioStageAt=0;kayakoStageAt=0;sadakoStageAt=0;[toshio,kayako,sadako].forEach(ghost=>{ghost.visible=false;ghost.position.y=.05;ghost.rotation.set(0,0,0);ghost.scale.setScalar(1);ghost.userData.pounce=null;ghost.userData.stunnedUntil=0;ghost.userData.feeding=0;});toshio.position.set(-7,.05,18-runLength*.315);kayakoStairs.position.set(8,0,18-runLength*.505);kayakoStairs.visible=false;cursedTelevision.position.set(-10,0,18-runLength*.69);cursedTelevision.visible=false;cursedTelevision.userData.screenMat.opacity=.82;sadako.userData.nextTeleport=0;
...
function end(win){
  state=win?'win':'caught';recordScore(win);clearTimeout(hauntTimer);witnessBubble.hidden=true;superCowBubble.hidden=true;watchers.forEach(w=>w.visible=false);[snowGhost,toshio,kayako,sadako,kayakoStairs,cursedTelevision].forEach(actor=>actor.visible=false);snow.visible=false;document.body.classList.remove('playing','death-maul','exhausted','enemy-near','terror-flash','flash-negative','apparition','blackout','blood-flash','heartbeat','otherworld','snow-haunting','rain-active','safe-warm','safe-cold','safe-broken');ui.result.classList.add('show');
...
function addClassicGhostChaser(ghost,speed){if(!activeChasers.includes(ghost)){ghost.visible=true;ghost.userData.chaseSpeed=speed;ghost.userData.pounce=null;ghost.userData.nextPounce=elapsed+1.2;activeChasers.push(ghost);}}
function updateClassicGhostSetpieces(progress,dt,t,safeProtected){
  if(toshioStage===0&&progress>runLength*.305){toshioStage=1;toshioStageAt=elapsed;toshio.visible=true;terrorFlash();playGhostSignature('toshio');say('ghost.toshioAppears',2900);}
  ...
  if(kayakoStage===0&&progress>runLength*.495){kayakoStage=1;kayakoStageAt=elapsed;kayakoStairs.visible=true;kayako.visible=true;terrorFlash();playGhostSignature('kayako');say('ghost.kayakoStairs',3400);}
  if(kayakoStage===1){const p=THREE.MathUtils.clamp((elapsed-kayakoStageAt)/3.5,0,1),introY=THREE.MathUtils.lerp(2.65,.08,p);kayako.position.set(kayakoStairs.position.x,introY,THREE.MathUtils.lerp(kayakoStairs.position.z-5.4,kayakoStairs.position.z+1.2,p));kayako.rotation.y=0;animateClassicGhost(kayako,t*1.25);kayako.position.y=introY;if(p>=1){kayakoStage=2;kayakoStairs.visible=false;addClassicGhostChaser(kayako,6.85);say('ghost.kayakoChases',2400);}}

  if(sadakoStage===0&&progress>runLength*.68){sadakoStage=1;sadakoStageAt=elapsed;cursedTelevision.visible=true;terrorFlash();playGhostSignature('sadako');say('ghost.television',3000);}
  ...
  if(sadakoStage===2){const p=THREE.MathUtils.clamp((elapsed-sadakoStageAt)/3.2,0,1);sadako.position.set(cursedTelevision.position.x,THREE.MathUtils.lerp(.05,.14,p),THREE.MathUtils.lerp(cursedTelevision.position.z-5.6,cursedTelevision.position.z+3.4,p));sadako.rotation.x=THREE.MathUtils.lerp(-1.25,0,p);sadako.scale.setScalar(.76+p*.24);animateClassicGhost(sadako,t);cursedTelevision.userData.screenMat.opacity=.45+Math.random()*.55;if(p>=1){sadakoStage=3;sadako.userData.nextTeleport=elapsed+5.5;addClassicGhostChaser(sadako,7.15);terrorFlash();}}
  if(sadakoStage===3&&sadako.visible&&!safeProtected&&elapsed>=sadako.userData.nextTeleport){const d=Math.hypot(sadako.position.x-player.position.x,sadako.position.z-player.position.z);sadako.userData.nextTeleport=elapsed+7+Math.random()*5;if(d>15){sadako.position.set(player.position.x+(Math.random()<.5?-1:1)*(7+Math.random()*5),.08,player.position.z-16-Math.random()*8);sadako.userData.stunnedUntil=elapsed+.7;terrorFlash();playGhostSignature('sadako');say('ghost.sadakoTeleports',2200);}}
