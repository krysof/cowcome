...
function triggerHaunt(){
  ...
  else if(kind===3){const w=watchers.find(x=>!x.visible)||watchers[0],side=Math.random()<.5?-1:1;w.position.set(player.position.x+side*(7+Math.random()*9),0,player.position.z-12-Math.random()*20);w.rotation.y=side>0?-.4:.4;w.userData.spoken=false;w.visible=true;horrorSound(0);hauntTimer=setTimeout(()=>w.visible=false,3200+Math.random()*1800);}
...
function tick(){
  ...
  if(state==='playing'){
    ...
    let collided=false;for(const o of obstacles){if(o.userData.collisionDisabled)continue;const radius=(o.userData.collisionRadius||2.1)+1.05,dx=player.position.x-o.position.x,dz=player.position.z-o.position.z,d2=dx*dx+dz*dz;if(d2<radius*radius){const d=Math.max(.001,Math.sqrt(d2)),push=radius-d;player.position.x+=dx/d*push;player.position.z+=dz/d*push;collided=true;}}
    ...
    if(!safeProtected)for(const snake of snakes){
      ...
      const d=Math.hypot(player.position.x-snake.position.x,player.position.z-snake.position.z);nearest=Math.min(nearest,d);if(d<4.2){stamina=Math.max(0,stamina-25*difficulty.hazard*dt);player.position.x+=(player.position.x-snake.position.x)*dt*2.5;shake=.28;damageHeart('event.snakeHit',snake,'death.snake');}
    }
    ...
    resolvePlayerAnimalCollision();resolveWatcherCollision();
